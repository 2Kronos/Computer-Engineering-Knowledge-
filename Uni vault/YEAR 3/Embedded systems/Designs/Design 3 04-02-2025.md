...
```c++
// Global variables
bool userIn;           // Current reading from the button.
bool user_prev = false;  // Previous button state (initialized to false).
bool led_on = false;     // Current LED state (off by default).

void setup() {
  pinMode(13, OUTPUT);
  pinMode(10, OUTPUT);
  pinMode(2, INPUT);     // Button connected to digital pin 2.
  pinMode(3, OUTPUT);    // LED connected to digital pin 3.
  Serial.begin(9600);
}

void loop() {
  // The following code is just to create a repeating pattern
  // with delays (h_delay) so you can see the LED state maintained.
  digitalWrite(13, HIGH);
  digitalWrite(10, LOW);
  h_delay(400);
...
// h_delay creates an approximate delay while checking for button presses
void h_delay(int time) {
  for (int c = 0; c < time; c++) {
    delay(1);                      // Wait for 1 millisecond.
    userIn = digitalRead(2);       // Read the current button state.

    // If the button is pressed now but was not pressed before,
    // then a rising edge has been detected.
  ...
  // Update the LED output without using the ternary operator.
 if (led_on == true) {
    digitalWrite(3, HIGH);
  } else {
    digitalWrite(3, LOW);
