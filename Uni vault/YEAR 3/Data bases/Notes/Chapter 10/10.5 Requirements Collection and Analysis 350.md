
#### **🔍 What is Requirements Collection & Analysis?**

The process of gathering and analyzing **organizational needs** to define the **database system’s functionality**.

#### **📌 Key Activities**

1. **Fact-Finding Techniques** (See Chapter 11)
    
    - Gather details for each **user view**:
        
        - **Data used/generated**
            
        - **How data is processed**
            
        - **Additional system requirements**
            
2. **Requirements Specification**
    
    - Convert informal requests into **structured documentation**.
        
    - Tools used:
        
        - **Data Flow Diagrams (DFD)**
            
        - **Hierarchical Input Process Output (HIPO) charts**
            
        - **CASE tools** (for consistency checks)
            
        - **UML** (discussed in Section 27.8)
            

#### ⚠️ **Challenges**

- **Too little analysis** → Wasted resources on the wrong solution.
    
- **Too much analysis** → "Paralysis by analysis" (overcomplicating early stages).
    
- **Balancing functionality**:
    
    - **Too little** → Users reject the system.
        
    - **Too much** → System becomes complex and hard to maintain.

### 🏗️ Managing Multiple User Views

When there are several user views (e.g., Marketing, Stock Control, Personnel), you can handle their requirements in three ways:

#### 10.5.1 Centralized Approach

**Definition:**  
Merge all user-view requirements into a **single** set; build **one** global data model.

- **When to use:**
    
    - ==High overlap between views.==
        
    - System complexity is moderate.
        
- **Process Sketch:**
    

```mermaid
flowchart LR
  UV1[User View 1] --> M[Merge]
  UV2[User View 2] --> M
  UV3[User View 3] --> M
  M --> GlobalModel[Global Data Model]
```

- **Outcome:**  
    A **single** requirements document and data model covering all views.
    

---

#### 10.5.2 View Integration Approach

**Definition:**  
Keep each user-view requirement **separate**; build **local** data models, then **merge** into a global model.

- **When to use:**
    
    - Views have significant differences.
        
    - System complexity is high, justifying divide-and-conquer.
        
- **Process Sketch:**
    

```mermaid
flowchart LR
  UV1 --> LM1[Local Model 1]
  UV2 --> LM2[Local Model 2]
  UV3 --> LM3[Local Model 3]
  LM1 --> I[Integrate]
  LM2 --> I
  LM3 --> I
  I --> GlobalModel[Global Data Model]
```

- **Outcome:**  
    Multiple **local** models refined separately, then **integrated**.
    

---

#### 10.5.3 Combined Approach

**Definition:**  
Use **centralized** merging for some related views to form **intermediate** models, then apply **view integration** to those intermediates to build the final global model.

- **When to use:**
    
    - Mixed overlap and divergence among views.
        
    - Need both efficiency and manageability.
        
- **Process Sketch:**
    

```mermaid
flowchart LR
  subgraph GroupA
    UV1 --> LM1
    UV2 --> LM1
  end
  subgraph GroupB
    UV3 --> LM2
    UV4 --> LM2
  end
  LM1 --> Integrate
  LM2 --> Integrate
  Integrate --> GlobalModel
```

- **Outcome:**  
    **Local** models for clusters of views, then **integrated** into one comprehensive model.
    

---
#### **🎯 Key Takeaways**

- Requirements must be **clear, structured, and balanced**.
    
- **User views** drive the design—overlap vs. uniqueness dictates the approach.
    
- **Tools like DFDs and UML** help formalize requirements.
